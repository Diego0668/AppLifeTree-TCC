  ...
  Future<List<ProdutoModel>> listarProdutos() async {
    ...
    try {
      dynamic response = await dio.get(
          'http://localhost:8080/api/lifetree/funcionario/produtosApi',
          options: Options(contentType: 'application/json; charset=UTF-8'));
      if (response.statusCode == 200) {
        for (Map<String, dynamic> item in response.data) {
          ProdutoModel p = ProdutoModel();
          p.setId(item.values.elementAt(0));
          p.setNome(nome: item.values.elementAt(1));
          p.setQuantidade(quantidade: item.values.elementAt(3));
          p.setStatusProd(statusProd: item.values.elementAt(5));
          if (item.values.elementAt(7) != null)
            p.setImagem(base64.decode(item.values.elementAt(7)));
          else
